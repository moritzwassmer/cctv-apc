...
# This script runs a training for the R2+1d-mLSTM-UpsamplingDecoder with masked loss.
...
echo "$formatted_date"
export EXP_NAME_SHORT=bottleneck_mlstm
...
hydra.launcher.timeout_min=2800 \
hydra.launcher.name=$EXP_NAME_SHORT \
run=full_run \
run.mlflow_creds=$MLFLOW_CREDS \
run/model=_decoder \
run/model/loss=masked \
run.model.input.num_blocks=8 \
run.model.output.num_blocks=2 \
...
run.model.min_lr=0 \
run.model.lr=0.0005,0.0001 \
run/dm=cctv_vd2_cut2_STD_global \
run.dm.train_ds_conf.prob_time_reverse=0.5 \
run/trainer/callbacks=cp-seperate \
run/trainer/logger=batched_mlflogger \
run.trainer.logger.log_metrics_every=50 \
run.trainer.logger.experiment_name=$EXP_NAME \
run.trainer.max_epochs=700 \
